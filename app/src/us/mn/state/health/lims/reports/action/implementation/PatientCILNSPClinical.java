...
public class PatientCILNSPClinical extends HaitiPatientReport implements
		IReportCreator, IReportParameterSetter {
	...
	static {
		...
		analysisStatusIds.add(Integer.parseInt(StatusService.getInstance()
		...
		analysisStatusIds.add(Integer.parseInt(StatusService.getInstance()
				.getStatusID(AnalysisStatus.ReferredIn)));
		analysisStatusIds.add(Integer.parseInt(StatusService.getInstance()
	...
	protected String reportFileName() {
		...
		ConfigurationProperties configurationProperties = ConfigurationProperties
				.getInstance();
		String configName = configurationProperties
				.getPropertyValue(Property.configurationName);

		if (configName.equals("CI IPCI")) {
			reportFileName = "PatientReportCIIPCI";
		} else if (configName.equals("CI LNSP")) {
			reportFileName = "PatientReportCILNSP";
		} 
	...
	protected void createReportItems() {
		List<Analysis> analysisList = analysisDAO
				.getAnalysesBySampleIdAndStatusId(reportSample.getId(),
						analysisStatusIds);
		...
		for (Analysis analysis : analysisList) {
			...
			if (analysis.getTest() != null) {
				...
				Referral referral = referralDao
						.getReferralByAnalysisId(reportAnalysis.getId());
				if (referral != null) {
	...
	private void addReferredTests(Referral referral,
			HaitiClinicalPatientData parentData, boolean parentStillReferred) {
		List<ReferralResult> referralResults = referralResultDAO
				.getReferralResultsForReferral(referral.getId());

		for (int i = 0; i < referralResults.size(); i++) {
			if (referralResults.get(i).getResult() != null
					&&
					// if referral has been canceled then don't show referred
					// tests with
					// no results
					(parentStillReferred || !GenericValidator
							.isBlankOrNull(referralResults.get(i).getResult()
									.getValue()))) {
				// pick up note from 1st of possible multiple results
				String referralNote = getResultNote(referralResults.get(i)
						.getResult());
				...
				if (!GenericValidator.isBlankOrNull(testId)) {
					...
					String uom = getUnitOfMeasure(referralResult.getResult(),
							test);
					if (reportReferralResultValue != null) {
						data.setReferralResult(addIfNotEmpty(
								reportReferralResultValue, uom));
					}
					data.setTestRefRange(addIfNotEmpty(
							getRange(referralResult.getResult()), uom));
					data.setTestSortOrder(GenericValidator.isBlankOrNull(test
							.getSortOrder()) ? Integer.MAX_VALUE : Integer
							.parseInt(test.getSortOrder()));
					TestSection resultTestSection = getTestSection(referralResult);
					if (resultTestSection != null) {
						data.setSectionSortOrder(resultTestSection
								.getSortOrderInt());
						data.setTestSection(resultTestSection
								.getLocalizedName());
					}
				}

				if (GenericValidator.isBlankOrNull(reportReferralResultValue)) {
					sampleCompleteMap.put(reportSample.getAccessionNumber(),
							Boolean.FALSE);
					data.setResult(StringUtil
							.getMessageForKey("report.test.status.inProgress")
							+ (augmentResultWithFlag() ? getResultFlag(
									referralResult.getResult(), "A") : ""));
				} else {
					data.setResult(reportReferralResultValue
							+ (augmentResultWithFlag() ? getResultFlag(
									referralResult.getResult(), "A") : ""));
				...
				data.setHasRangeAndUOM(referralResult.getResult() != null
						&& "N".equals(referralResult.getResult()
								.getResultType()));
	...
	private void copyParentData(HaitiClinicalPatientData data,
			HaitiClinicalPatientData parentData) {
	...
	protected void postSampleBuild() {
		if (reportItems.isEmpty()) {
			...
			HaitiClinicalPatientDataColFormat colData = new HaitiClinicalPatientDataColFormat(
					reportItem);
			colData.setSectionName(StringUtil
					.getMessageForKey("report.no.results"));
	...
	private void buildReport() {
		Collections.sort(reportItems,
				new Comparator<HaitiClinicalPatientData>() {
					@Override
					public int compare(HaitiClinicalPatientData o1,
							HaitiClinicalPatientData o2) {
						String o1AccessionNumber = AccessionNumberUtil
								.getAccessionNumberFromSampleItemAccessionNumber(o1
										.getAccessionNumber());
						String o2AccessionNumber = AccessionNumberUtil
								.getAccessionNumberFromSampleItemAccessionNumber(o2
										.getAccessionNumber());
						int accessionSort = o1AccessionNumber
								.compareTo(o2AccessionNumber);

						if (accessionSort != 0) {
							return accessionSort;
						}

						int sectionSort = o1.getSectionSortOrder()
								- o2.getSectionSortOrder();
		...
		for (HaitiClinicalPatientData reportItem : reportItems) {
			// This handles the case where two orders in a row have a single
			// identical section
	...
	private String getRootAccessionNumber(HaitiClinicalPatientData reportItem) {
		...
		if (separatorIndex > 0) {
			itemAccessionNumber = itemAccessionNumber.substring(0,
					separatorIndex);
		}
		return itemAccessionNumber;
	}

	private HaitiClinicalPatientDataColFormat buildCol(
			HaitiClinicalPatientDataColFormat colData,
			HaitiClinicalPatientData patientData, int column) {
		if (column == 1) {
			...
			String accessionNumber = AccessionNumberUtil
					.getAccessionNumberFromSampleItemAccessionNumber(patientData
							.getAccessionNumber());
			colData.setCompleteFlag(StringUtil.getMessageForKey(sampleCompleteMap
					.get(accessionNumber) ? "report.status.complete"
	 ...
	 */
	private void buildTestSection(List<HaitiClinicalPatientData> testSection,
			HaitiClinicalPatientDataColFormat colData) {
		// calculates half # of report Items, if an odd number adds 1 to account
		// for truncated division
		int half = (testSection.size() % 2 == 0) ? testSection.size() / 2
				: testSection.size() / 2 + 1;
	...
	private String createReadableAge(String dob) {
		...
		int months = DateUtil.getAgeInMonths(dobDate,
		...
		if (months > 35) {
			...
			int days = DateUtil.getAgeInDays(dobDate,
					DateUtil.getNowAsSqlDate());
			return days + " J";
		}
	...
	public JRDataSource getReportDataSource() throws IllegalStateException {
		...
		return errorFound ? new JRBeanCollectionDataSource(errorMsgs)
				: new JRBeanCollectionDataSource(clinicalReportItems);
	}
	...
	@Override
	protected void setReferredResult(HaitiClinicalPatientData data,
			Result result) {
		data.setResult(data.getResult()
				+ (augmentResultWithFlag() ? getResultFlag(result, "R") : ""));
