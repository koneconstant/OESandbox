...
public class AnalyzerLineReader extends AnalyzerReader {
	...
	private static final CharSequence GERBER_4000_INDICATOR = "VIH_P";
	...
	private void setInserter() {
		// This is going to be highly customized based on the characteristics of
		// the file
		// being sent

		if (lines.get(0).contains(COBAS_INDICATOR)) { // Cobas is found on the
														// first line
			inserter = new CobasReader();
		} else if (lines.get(1) != null
				&& lines.get(1).contains(GERBER_4000_INDICATOR)) {	 
			inserter = new Gerber4000Reader();
				...
				|| lines.get(0).contains(EVOLIS_MUREX_DBS_INDICATOR)) { // Evolis
																		// is
																		// found
																		// on
																		// the
			...
			inserter = new EvolisReader();
				...
				&& lines.get(1).contains(SYSMEX_XT_INDICATOR)) { // Sysmex model
																	// found on
																	// data line
			inserter = new SysmexReader();
		} else if (lines.get(1) != null
				&& lines.get(1).contains(FACSCALIBUR_INDICATOR)) { // Fascalibur
																	// software
																	// found on
																	// data line
			inserter = new FacscaliburReader();
				...
				&& (lines.get(1).contains(COBAS_TAQMAN_INDICATOR)
						|| lines.get(1).contains(COBAS_TAQMAN_INDICATOR_2) || lines
						.get(1).contains(COBAS_TAQMAN_INDICATOR_3))) {
			inserter = new CobasTaqmanReader();
		} else if (lines.get(1) != null
				&& lines.get(1).contains(FACSCANTO_INDICATOR)) {
			inserter = new FACSCantoReader();
		} else if (lines.get(1) != null
				&& lines.get(1).contains(COBAS_TAQMAN_DBS_INDICATOR)) {
			inserter = new CobasTaqmanDBSReader();
		} else {
			// we're into squishy territory. It could be be TAQMAN with no test
			// on first row
			for (String line : lines) {
				if (line.contains(COBAS_TAQMAN_INDICATOR)
						|| line.contains(COBAS_TAQMAN_INDICATOR_2)) {
					inserter = new CobasTaqmanReader();
				}
			}
